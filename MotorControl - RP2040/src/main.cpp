...
// PID
  ...
  // Position
    ...
    double Pos_Kp = 3.0; // 1.5
    double Pos_Ki = 0.0; // 0.3
    double Pos_Kd = 0.0; // 0.1
    ArduPID PID_Pos;
    #define POS_SETPOINT_LIMITS 552
    #define POS_WINDUP_LIMITS  2500
    #define POS_OUTPUT_LIMITS 5000
  // Speed
...
void loop() {
  ...
  if (millis() - PID_sample_last >= SAMPLE_PERIOD) {
    ...
    PID_Pos_Input = (double)enc_position;
  ...
  //if (millis() - loop_display_last >= DISPLAY_WAIT_TIME) {
    if (header_display >= HEADER_COUNT) {
      ...
      Serial.println("Time [ms] | Pos_Setpoint | Pos_Position | Pos_Output | Speed_Setpoint | Speed Input | Speed_Output");
      header_display = 0;
    } else {
      header_display++;
    }
    ...
    Serial.print(millis());
      Serial.print("|");
      Serial.print((int)PID_Pos_Setpoint);
      Serial.print("|");
      Serial.print(PID_Pos_Input, 0);
      Serial.print("|");
